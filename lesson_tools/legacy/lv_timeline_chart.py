...
def _resolve_student_name(value, id_to_name):
    s = str(value).strip()
    return id_to_name.get(s, s)


def create_visualizations(data, student_data=None, id_to_name=None):
    ...
    if has_student_data:
        ...
        students_answering = {}
        for q in interactions['teacher-question']:
            for raw in q['answered_by']:
                name = _resolve_student_name(raw, id_to_name)
        ...
        for s in student_data:
            ...
            event_dt = s.get('follow_dt')
            if event_dt is None:
                continue
            mistakes = [(kw, dt) for kw, dt in events
                        if _positionable_event(kw, dt)]
            if mistakes:
                kw_label = min(mistakes, key=lambda e: e[1])[0]
            elif events:
                kw_label = events[0][0]
            else:
                kw_label = '(followed till end)'
            ...
            if not closest or closest[0] > _THRESH:
                ...
                if closest_q:
                    ...
                    if qtype == 'teacher-question':
                        ...
                        answerers = [_resolve_student_name(v, id_to_name)
                                     for v in q.get('answered_by', [])]
                        ...
                        askers = [_resolve_student_name(v, id_to_name)
                                  for v in q.get('asked_by', [])]
                        question_text = q.get('info', '').strip()
                        txt = f"Q: {question_text}" if question_text else "Q: (student question)"
                        if askers:
                            names = "\n".join(f"  • {name}" for name in askers)
                            txt += f"\n\nAsked by:\n{names}"
                    elif qtype == 'providing-help':
                        helpers = [_resolve_student_name(v, id_to_name)
                                   for v in q.get('student', [])]
                        txt = "Providing help"
                        if helpers:
