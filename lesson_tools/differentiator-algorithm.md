...
LEO-js differentiator to compare a student's submitted code against a
...
keystroke and the timestamp at which it was typed. Students submit final
...
<grading_root>/
  ...
  <category>/
    <project>/
      ...
      anon_ids/               — anonymised submissions keyed by student id;
      ...
      log.json                — the keylog (name can vary, only JSON file there)
```
...
from `token_log_leo.py`, `token_log_marks.py`, and `token_log_starpass.py`,
so existing `from utils.token_log import _foo` import paths still resolve
unchanged.
