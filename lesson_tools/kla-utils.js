...
function lowerBound(arr, val, key) {
  ...
  while (lo < hi) { const mid=(lo+hi)>>1; if (key(arr[mid]) < val) lo=mid+1; else hi=mid; }
  return lo;
}
...
function charsAt(ts_secs, cum) { return upperBound(cum, ts_secs, c => c.ts); }
...
function niceStep(max, steps) {
  const rough = max/steps, mag = Math.pow(10, Math.floor(Math.log10(rough))), n = rough/mag;
