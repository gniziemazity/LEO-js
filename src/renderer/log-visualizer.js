...
class TextState {
	...
	deleteBack(n = 1) {
		...
		this.text =
			this.text.slice(0, this.cursor - n) + this.text.slice(this.cursor);
	...
	deleteForward(n = 1) {
		...
		this.text =
			this.text.slice(0, this.cursor) + this.text.slice(this.cursor + n);
...
function splitCodeWithAnchors(code) {
	...
	while ((m = ANCHOR_RE.exec(code)) !== null) {
		if (m.index > last)
			segs.push({ kind: "text", val: code.slice(last, m.index) });
...
function expandEvents(events) {
	...
	for (let i = 0; i < n; i++) {
		...
		if ("char" in ev) {
			...
			const totalChars = segs
				.filter((s) => s.kind === "text")
				.reduce((a, s) => a + s.val.length, 0);
			let charI = 0;
...
class LogVisualizer {
	...
	loadFile(filePath) {
		...
		this._setStatus(
			`  Loaded: ${name}  ·  ${events.length} events → ${this.micro.length} steps`,
		);
	}
	...
	_handleChar(ch, ts, delay, editor) {
		...
		if (ch === "⌫") {
			...
			if (
				(prevChar === "\n" || prevChar === "") &&
				CLOSING.some((t) => nextText.startsWith(t))
			) {
				this._log(
					ts,
	...
	_autoDedent(ch, ts) {
		...
		this.main.text =
			this.main.text.slice(0, lineStart) +
			newBefore +
			this.main.text.slice(this.main.cursor);
